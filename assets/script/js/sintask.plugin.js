...
/**
 * fn.sintaskDivPlaceholder = add placeholder on div contenteditable
 */
sjqNoConflict.fn.sintaskDivPlaceholder = function() {
    ...
    if(checkThisPlaceholder<1) {
        sjqNoConflict(this).parent().prepend("<div class='typeDivPlaceholder unSelectAble'>"+thisPlaceholder+"</div>");
    }
...
/**
 * fn.sintaskDsTagsTextarea = show DoubleSlashTag pre-view 
 * on input area (html element textarea)
 */
sjqNoConflict.fn.sintaskDsTagsTextarea = function(param) {
    sjqNoConflict(this).wrap('<div class="doubleSlashTagSintask"><div class="dstagThisbold" id="dstag'+param+'"></div></div>').unwrap().before('<div class="dstagThisbold" id="dstag'+param+'"></div>').wrap('<div class="typehead"></div></div>');
    sjqNoConflict(this).addClass("addTheContent");
    sjqNoConflict(this).on("keydown keyup", function() {
        var str = sjqNoConflict(this).val();
        sjqNoConflict(this).parent().parent().find(".dstagThisbold").css("width",sjqNoConflict(this).css("width"));
        str = str.replace(/\n/g, '<br>');
        str = str.replace(/(http|ftp|https):\/\/([a-zA-Z0-9@:%_\+.~\#?&//=]+)/g,'<span class="dsTag">$1:\/\/$2</span>');
        if(!str.match(/\/\/([a-zA-Z0-9_]+)\/\//g)) {
            str = str.replace(/\/\/([a-zA-Z0-9_]+)/g,'<span class="dsTag">\/\/$1</span>');
        } else {
            str = str.replace(/\/\/([a-zA-Z0-9_]+) \/\/([a-zA-Z0-9_]+)/g,'<span class="dsTag">\/\/$1</span>');
        }
        sjqNoConflict(this).parent().parent().find(".dstagThisbold").html(str);
    });
    sjqNoConflict(this).parent().prev().on('click', function() {
        sjqNoConflict(this).parent().find(".addTheContent").focus();
    });
};
/**
 * fn.sintaskTooltipHumanTime = show tooltip from realtime time
 * time is get from timestamp-sintask atribute on html element to be human readable
 */
sjqNoConflict.fn.sintaskTooltipHumanTime = function() {
    sjqNoConflict(this).hover(function(){
            var thisval = sjqNoConflict(this).attr("timestamp-sintask");

            thisval = timeStampToHumanTime(thisval, "noday");

            var getThisTooltipLen = sjqNoConflict("#sintask_tooltip_humantime").length;
            if(getThisTooltipLen<1) {
               sjqNoConflict(this).append("<div id='sintask_tooltip_humantime' class='p_ab st_tooltip_2'>"+thisval+"</div>");
            }
        }, function(){
            sjqNoConflict("#sintask_tooltip_humantime").remove();
            sjqNoConflict(".st_tooltip_2").remove();
    });
    sjqNoConflict(this).on("click", function(){
        var thisvalonclick = sjqNoConflict(this).attr("timestamp-sintask-onclick");
        if(thisvalonclick!="1") {
            sjqNoConflict("#sintask_tooltip_humantime").remove();
            sjqNoConflict(".st_tooltip_2").remove();
        }
...
/**
 * fn.sintaskInfoHumanTime = show info from realtime time (show info to html tag)
 * time is get from timestamp-sintask atribute on html element to be human readable
 */
sjqNoConflict.fn.sintaskInfoHumanTime = function() {
    var thisval = sjqNoConflict(this).attr("timestamp-sintask");
    thisval = timeStampToHumanTime(thisval, "noday");
    sjqNoConflict(this).html(thisval);
    return this;
};
/**
...
/**
 * fn.sintaskDropDownMenu = show drop down menu
 * not active now !!!
 */
sjqNoConflict.fn.sintaskDropDownMenu = function(arraychoice) {
    var randId = getRandomOnSinTask(2);
    var thisHtml = "<div id='dropDown"+randId+"' class='sintaskDropDown'>";
    for(a = 0; a < arraychoice.length; a++) {
        if( arraychoice[a]['id'] != null && 
            arraychoice[a]['id'] != "" &&
            ctypeSpace(arraychoice[a]['id']) > 0
        ) {
            thisHtml    += "<div id="+arraychoice[a]['id']+" class='choiceSintaskDropDown'>"
                            + "<div class='icon"+arraychoice[a]['icon']+"'>"
                                + arraychoice[a]['menu']
                            + "<\/div>"
                        + "<\/div>";
        }
    ...
    thisHtml += "<\/div>";
    sjqNoConflict(this).html(thisHtml);
    return this;
};
/**
