...
var __SFWspa = function () {
  ...
  var __SFW_250ErrorDesc =
    '[<b>Kode Error 250</b>]<br>Ada kesalahan dengan __SFW_tokenizing karena tidak dapat diverifikasi oleh sisi Server, silahkan re-load halaman ini, jika terus berlanjut Selengkapnya buka <a class="ft_style_u" href="https://fw.sintask.com/docs/error">Dokumentasi Error SinTaskFW</a>';
  var __SFW_251ErrorDesc =
    "[<b>Kode Error 251</b>]<br>Ada beberapa kesalahan yang mungkin terjadi : <br><br>1. Saat mengambil data dari Server, kemungkinan SPA tidak dapat membaca JSON karena Error, periksa lagi setiap baris kode anda. <br><br>2. Terjadi perpindahan halaman dari halaman berbasis SPA ke halaman normal (Not-SPA), silahkan periksa perpindahan halaman anda.";
  ...
  function sintaskLoaderIframeStop() {
    setTimeout(function () {
      ...
      if (
        typeof checkCss != "undefined" &&
        checkCss != "" &&
        __SFW_f.ctypeSpace(checkCss) != 0
      ) {
        ...
        if (typeof __SFW_globalScrollPage[document.URL] === "undefined") {
          ...
          sjqNoConflict(document).scrollTop(
            __SFW_globalScrollPage[document.URL]
          );
        }
      } else {
        sintaskLoaderIframeStop();
  ...
  function loadAddStyle(link, id) {
    if (link != "null") {
      ...
      if (sjqNoConflict("." + idfinal).length > 0) {
        ...
        sjqNoConflict("body").append(
          "<style class='" + idfinal + " styleAdd iDisplayNone'></style>"
        );
        setTimeout(function () {
          loadAddStyle(link, id);
        }, 200);
      }
    }
    return true;
  ...
  function sintaskSPA(input) {
    ...
    var replacer = ["http://", "https://", "ftp://", "ftps://", ":"];
  ...
  function __SFW_spaClick() {
    ...
    var allKeyboardEvent = ["keydown", "keypress", "keyup"];
    ...
    var itAll = 0;
    var mergeEvent = allMouseEvent
      .concat(allKeyboardEvent)
      .concat(allDOMEelementEvent);
    ...
    sjqNoConflict(document).on("click", ".s, a[spa]", function (e) {
      ...
      if (pageUrl == "" || pageUrl == null || typeof pageUrl == "undefined") {
        pageUrl =
          sjqNoConflict(this).attr("s-data-url") ||
          sjqNoConflict(this).attr("spa-url");
      }
